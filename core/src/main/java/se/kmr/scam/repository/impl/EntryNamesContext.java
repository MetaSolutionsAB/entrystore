...
public class EntryNamesContext extends ContextImpl {
	...
	protected boolean setEntryName(URI entryURI, String newName) {
		...
		if (forEntry == null) {
			...
			throw new se.kmr.scam.repository.RepositoryException("Unable to set name for non-existing entry");
		}
		...
		try {
			...
			throw new se.kmr.scam.repository.RepositoryException("Cannot connect to repository", e);
		}
	}
