...
[ENTRYSTORE-835](https://metasolutions.atlassian.net/browse/ENTRYSTORE-835) Add language information to facet values. Faceting on the new metadata.predicate.literal_l.* and related.metadata.predicate.literal_l.* Solr fields returns one bucket per label and language tag; each value carries a "lang" key beside "name" and "count" when the literal has a language tag, and facetMatches on these fields matches the label. The existing literal_s, literal_t and literal facet output is unchanged. **Upgrade note:** deploy the updated Solr schema.xml and reload the core before starting 6.1. The indexer writes the new fields for every literal, and a schema without them rejects the whole document as an unknown field. The index is rebuilt automatically on first boot by the Solr version marker; a developer instance already running a 6.1-SNAPSHOT needs one start with entrystore.solr.reindex-on-startup=on.
