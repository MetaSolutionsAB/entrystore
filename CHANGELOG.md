...
[ENTRYSTORE-1058](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1058) Deduplicate core-impl code and remove dead configuration surface. ContextImpl's four create methods \(createLink, createReference, createLinkReference, createResource\) had identical bodies apart from one argument list, each opening its own synchronized block on the repository; they now share a single private create method, so the lock scope and the two-tier list-versus-context authorization are stated once. The GraphType post-processing that only createResource needs runs through a hook inside the same monitor, so it is still not observable as a separate critical section. ListImpl's \_moveEntryHere and \_copyEntryHere each carried a verbatim copy of the switch over EntryType that dispatches into those four methods; both now call one createCopyHere helper. ContextManagerImpl.getLinksOrReferences, a boolean-flag method whose two branches differed only in the index predicate and the wanted EntryType, is replaced by getEntriesFromIndex taking both as parameters, and its RepositoryResult is now closed by try-with-resources rather than by hand. EntryUtil's sortAfterModificationDate and sortAfterCreationDate, previously identical except for the date getter, delegate to one private sortAfterDate taking the getter as a Function; the existing null-entry-compares-equal behaviour is preserved deliberately, so the comparator is not a total order and a null entry still neither throws nor is ordered. The hand-rolled last-path-segment idiom is replaced by URISplit.getLastSegment at 20 call sites across core-impl and the REST layer, each keeping its own toString\(\)/toASCIIString\(\) rendering and the existing edge-case behaviour \(a trailing slash yields an empty string, a slash-free input is returned unchanged\); nine of those sites had been performing the URI-to-string conversion twice in one expression. Seven Settings constants referenced by no code, no properties file and no example file are removed: STORE\_DBNAME, STORE\_PORTNR, STORE\_SERVERNAME, STOREJS\_JS, STOREJS\_CSS, HARVESTER\_FAO and HARVESTING\_TARGET\_OAI\_BASE\_URI. Two robustness fixes fall out of the consolidation: ContextImpl.getList now reports a listURI that names no entry instead of throwing NullPointerException \(a listURI naming something that is not a local list is still ignored rather than rejected, which is unchanged\), and getLinks/getReferences now skip an index triple that resolves to no entry, with a warning naming it, instead of failing the whole call on one stale triple. No REST-observable behaviour changes; the only externally visible difference is that createResource no longer logs two INFO lines per entry created into a list
