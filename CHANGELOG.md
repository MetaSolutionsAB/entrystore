...
[ENTRYSTORE-1056](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1056) Decompose the REST layer's resource controller and service. ResourceController no longer branches on graph type, converts feeds or instantiates core classes: GET representation dispatch, feed rendering, digest reading and modification-date bumps live in ResourceService and SyndicationService; the user-settings update \(name, password with the current-password challenge, session expiry and confirmation email, language, home context, disabled flag, custom properties\) moves out of ResourceService into a dedicated UserSettingsService, and the step that follows every successful password change is shared with the password-reset flow; the search limit clamp and filterQuery parsing move from SearchController into SolrSearchInputValidator. No status code or client-visible message changes apart from three fixes in code the refactor touched: a String resource without an rdf:value answers 200 with an empty body again instead of 500, a binary resource whose stored mimetype is invalid is served as application/octet-stream with a warning instead of failing every GET with 500, and a malformed percent-encoding in the filterQuery parameter of GET /search answers 400 instead of 500
