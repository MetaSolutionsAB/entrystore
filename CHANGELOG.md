...
[ENTRYSTORE-1054](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1054) Send email through Spring's JavaMailSender instead of a static per-call jakarta.mail Session. Security note: transport security is entrystore.smtp.security — every earlier entrystore.properties\_example documented entrystore.smtp.ssl instead, a key the code never read, so deployments that followed the documentation were silently sending plaintext SMTP. Both spellings are now honoured: security is used whenever it resolves, ssl only as a fallback, and if both are set and disagree, security applies and the deprecated key is reported at WARN. Neither key was validated before 6.1 — ssl was never read at all, and security \(defined since 4.8\) was compared with equalsIgnoreCase — so an unrecognised value in either is reported at ERROR and resolves to off, which is exactly what such a deployment already did, and a configured host that ends up on plaintext is now warned about at startup rather than passing silently. **Upgrade note:** transport security is now enforced rather than downgraded, so a deployment carrying a stale entrystore.smtp.ssl value will see sends start failing rather than continue insecurely. entrystore.smtp.ssl=ssl against the entrystore.smtp.port=587 the old example file documented alongside it now means implicit TLS on a submission port, and entrystore.smtp.ssl=starttls against a relay that does not advertise STARTTLS now throws instead of silently downgrading — signup, password reset and POST /message all stop working. Verify the MTA supports the implied mode before upgrading, and declare a deliberately plaintext relay explicitly with entrystore.smtp.security=off. A second security fix: a CR or LF in the subject of POST /message is now collapsed to a space, closing an email-header injection an authenticated caller could otherwise use to add headers to a message carrying the deployment's own From address, and the recipient and Reply-To of POST /message must now be exactly one address each — InternetAddress.parse accepts a comma-separated list and a principal name is never format-validated, so a renamed principal could previously relay attacker-authored mail to arbitrary third parties under the deployment's own From. Config note: entrystore.smtp.email.from|bcc|reply-to are now documented and supersede entrystore.auth.email.from|bcc, which remain readable as a deprecated fallback; the SMTP socket timeouts, TLS server-identity checking and total send-attempt count become configurable as entrystore.smtp.connection-timeout, .read-timeout, .write-timeout, .check-server-identity and .max-send-attempts \(1-10, default 3 — a total attempt count, not a retry count\), all defaulting to the values previously compiled in; entrystore.smtp.username and entrystore.smtp.password must both be set, and a half-set pair is reported at ERROR and disables SMTP authentication rather than failing startup
