...
[ENTRYSTORE-1058](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1058) Deduplicate core-impl and fix the contracts the audit found broken. Consolidated into one implementation each: ContextImpl's four create methods, ListImpl's two EntryType switches, ContextManagerImpl.getLinksOrReferences, EntryUtil's two date sorts, the last-path-segment idiom \(URISplit.getLastSegment, 22 sites\) and the IRI-rewrite loops \(ModelUtil\); seven unreferenced Settings constants removed. **Behaviour note:** POST /\{context-id\} with a list parameter naming no entry answers 400 instead of 500, and one naming an entry that is not a list is ignored instead of answering 500 after the entry was created. getLinks/getReferences skip an unusable index triple with one bounded warning instead of failing the call. SearchIndex.reindex, reindexSync, reindex\(URI\) and reindexSync\(URI\) lose the purgeAllBeforeReindex parameter, which no caller ever set. Entry.setResourceURI refuses to change a local entry's resource URI \(400\): it is derived from the id and keys the resource's own graph and its children's membership, and renaming it overflowed the stack before. A Link, Reference or LinkReference rename keeps the type and resource-level ACL triples, updates inverse relations, fires EntryUpdated so the entry is re-indexed, and answers 500 instead of reporting success when the store fails; PUT /\{context-id\}/entry/\{entry-id\} rewrites the old resource URI throughout the submitted graph. Entry.removeAllowedPrincipalsFor returns whether the principal was listed; Entry.hasAllowedPrincipals throws RepositoryException instead of NullPointerException when the store is unreachable; EntryImpl.updateAllowedPrincipalsFor is removed
