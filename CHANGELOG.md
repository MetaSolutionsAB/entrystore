...
[ENTRYSTORE-1060](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1060) Cache each user's group memberships across authorization decisions. Every access check used to resolve the caller's groups by iterating all entries in the \_principals context and testing membership on every group — twice per ACL evaluation and up to twelve times per entry serialization — so the cost of authorizing an authenticated request grew linearly with the size of the user directory. The core now keeps a per-repository map from user to group set, consulted on every decision and invalidated by a repository-event listener: a post-commit event sourced from a group entry \(membership add or remove, a replacement of the member list, group create or delete, and a failed member-list write\) invalidates every cached set, and a deleted user drops its own entry, so a revoked membership is denied on the very next request. Each cached set is stamped with the invalidation epoch it was computed under and is served only while that epoch is current, and only a scan taken over a complete principals index is cached, so neither a scan that overlapped a membership change nor a short listing can outlive its cause. Repository-event dispatch is now failure-isolated: a listener that throws is logged and the remaining listeners still run. Measured on the same design under ENTRYSTORE-1085 at a 2 207-principal directory: group-authorized reads 16.6x faster \(1.69 ms to 0.10 ms per decision\), on par with admin reads, with no measurable write-path cost. A member-list write that fails restores the in-memory list and invalidates the cache before it attempts recovery, a context import drops a pruned list's in-memory members before publishing the change, and a principals index mapping without an entry no longer prevents the scan from being cached. **Config note:** entrystore.auth.group-cache \(default true\) is the kill switch; false restores the per-decision scan
