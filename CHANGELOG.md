...
[ENTRYSTORE-1058](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1058) Deduplicate the core-impl entry-creation, list-dispatch, index-lookup, sorting and URI-segment code, and remove dead configuration surface. ContextImpl's four create methods \(createLink, createReference, createLinkReference, createResource\) had identical bodies apart from one argument list, each opening its own synchronized block on the repository; they now share a single private create method, so the lock scope and the two-tier list-versus-context authorization are stated once. The GraphType post-processing that only createResource needs still runs inside that same monitor, and the dead allowUserGroupToReadMetadata local with its unreachable else branch is gone. ListImpl's \_moveEntryHere and \_copyEntryHere each carried a verbatim copy of the switch over EntryType that dispatches into those four methods; both now call one createCopyHere helper. ContextManagerImpl.getLinksOrReferences, a boolean-flag method whose two branches differed only in the index predicate and the wanted EntryType, is replaced by getEntriesFromIndex taking both as parameters, and its RepositoryResult is now closed by try-with-resources rather than by hand. EntryUtil's sortAfterModificationDate and sortAfterCreationDate, previously identical except for the date getter, delegate to one private sortAfterDate taking the getter as a Function; the comparator itself is unchanged, so it still returns 0 against a null entry rather than ordering it. The hand-rolled last-path-segment idiom is replaced by URISplit.getLastSegment at 22 call sites across core-impl, the REST layer and the integration tests, each keeping its own toString\(\)/toASCIIString\(\) rendering and the existing edge-case behaviour \(a trailing slash yields an empty string, a slash-free input is returned unchanged\); nine of those sites had been performing the URI-to-string conversion twice in one expression. The one remaining copy, in the OAI-PMH harvester, is left untouched because both harvesting submodules are commented out of their POM, so the change cannot be compile-verified. Seven Settings constants referenced by no code, no properties file and no example file are removed: STORE\_DBNAME, STORE\_PORTNR, STORE\_SERVERNAME, STOREJS\_JS, STOREJS\_CSS, HARVESTER\_FAO and HARVESTING\_TARGET\_OAI\_BASE\_URI. Two robustness fixes fall out of the consolidation, and both are REST-observable on error paths that previously answered 500. A listURI that names no entry was dereferenced unguarded and threw NullPointerException; it is now rejected as invalid input and answers 400, and because that rejection happens only after the authorization check, a caller who is denied can no longer tell an absent list from a present one \(a listURI naming something that is not a local list is still ignored rather than rejected, which is unchanged\). And getLinks/getReferences now skip an index triple they cannot use — one whose object is not an IRI, that java.net.URI rejects, or that does not sit under the current repository base URL, as a restored backup or a changed entrystore.baseurl.folder leaves behind — reporting the count once per call, with a bounded sample of the offending objects, where one such triple previously failed the whole call with an exception that surfaced as 500; a metadata traversal over a store holding stale index triples therefore now answers 200 with the entries it could resolve. Otherwise no REST-observable behaviour changes; the only other externally visible difference is that createResource no longer logs two INFO lines per entry created into a list

[ENTRYSTORE-835](https://metasolutions.atlassian.net/browse/ENTRYSTORE-835) Add a facetLang parameter to /search?type=solr, so a client can show facets for one language. Facet buckets are unchanged in shape \(a name and a count\) and a request without facetLang sends Solr exactly the query it sent before. With facetLang set, the buckets of a literal facet field \(metadata.predicate.literal\_s.\* or the metadata.predicate.literal.\* shorthand, and their related. twins\) are narrowed to the labels occurring in that language or untagged, using RFC 4647 basic filtering so en also matches en-GB. The selection happens in Solr before facetLimit, and the counts returned are exact; because the candidates are gathered with a bounded overrequest, the last places of the top N are approximate, which is the same contract Solr's own distributed faceting gives. Labels longer than 256 characters carry no language term and are therefore absent whenever facetLang is set. Internally the indexer writes one metadata.predicate.literal\_l.\* term per \(label, language\) pair, normalised with RDF4J Literals.normalizeLanguageTag so en-gb and en-GB share a term; that family is never accepted in facetFields or sort and never appears in a response. Startup now refuses to run against a Solr whose schema lacks those dynamic fields, or that answers 401/403 when asked for the schema, instead of wiping the index and rebuilding it empty; an unreachable Solr keeps the previous behaviour. Also fixed: a query needing more than one result-fill pass reported each facet field twice and asked Solr to recompute the facets on every pass. **Upgrade note:** deploy the updated Solr schema.xml and reload the core before starting 6.1. The indexer writes the new fields for every literal, and a schema without them rejects the whole document as an unknown field. With entrystore.data.folder configured the Solr version marker rebuilds the index on first boot; without it, and for a developer instance already running a 6.1-SNAPSHOT, start once with entrystore.solr.reindex-on-startup=on.
