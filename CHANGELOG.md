...
[ENTRYSTORE-1190](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1190) Remove entrystore.trust.x-forwarded-for, which had no effect: with server.forward-headers-strategy=framework, Spring's ForwardedHeaderFilter already makes request.getRemoteAddr\(\) the leftmost X-Forwarded-For entry and removes the header, so the sign-up, password-reset and search rate limiters were keyed by X-Forwarded-For with either setting. They now use request.getRemoteAddr\(\) directly. **Deployment note:** the client IP comes from Forwarded \(for=\), else the leftmost X-Forwarded-For entry. The reverse proxy must drop client-supplied Forwarded and X-Forwarded-\* and set X-Forwarded-For to the connecting client's address instead of appending to it, otherwise clients can spoof the IP used for rate limiting, reCAPTCHA verification, lastUsedIpAddress and request logging. Without a reverse proxy, set server.forward-headers-strategy=none. A configured entrystore.trust.x-forwarded-for is ignored and logs a WARN at startup.

[ENTRYSTORE-1183](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1183) Make boolean EntryStore settings consistently accept true/on/yes/1 and false/off/no/0, case-insensitively, including SAML/CAS startup and HTTP logging. Settings read by core warn on an unrecognised value and use their default; typed Spring settings fail startup. Spring Boot and third-party keys (spring.*, server.*, management.*, springdoc.*) must use true/false. **Behaviour note:** true, yes and 1 now enable settings that only on enabled before, among them entrystore.solr, entrystore.data.quota, entrystore.repository.public, entrystore.repository.provenance, entrystore.cors, entrystore.auth.recaptcha, entrystore.backup.scheduler and entrystore.repository.store.init-with-test-data \(which loads test users with publicly known passwords\). entrystore.harvester.oai is now disabled by false/off/no/0 in any case and by an unrecognised value, where only a lowercase off disabled it before. For other settings read by core, an unrecognised value now falls back to the setting's default instead of false. An unrecognised value fails startup for entrystore.cors, entrystore.auth.recaptcha, entrystore.auth.signup.create-home-context, entrystore.backup.scheduler, entrystore.repository.store.init-with-test-data, entrystore.traversal.\<profile\>.repository-scope and logging.http.enabled, which treated it as false before; a blank value also fails startup for the first three.

### Improvement

[ENTRYSTORE-1058](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1058) Deduplicate core-impl and fix the contracts the audit found broken. Consolidated into one implementation each: ContextImpl's four create methods, ListImpl's two EntryType switches, ContextManagerImpl.getLinksOrReferences, EntryUtil's two date sorts, the last-path-segment idiom \(URISplit.getLastSegment, 22 sites\) and the IRI-rewrite loops \(ModelUtil\); seven unreferenced Settings constants removed. **Behaviour note:** POST /\{context-id\} with a list parameter naming no entry answers 400 instead of 500, and one naming an entry that is not a list is ignored instead of answering 500 after the entry was created. getLinks/getReferences skip an unusable index triple with one bounded warning instead of failing the call. SearchIndex.reindex, reindexSync, reindex\(URI\) and reindexSync\(URI\) lose the purgeAllBeforeReindex parameter. Entry.setResourceURI refuses to change a local entry's resource URI \(400\): it is derived from the id and keys the resource's own graph and its children's membership. Two entries may hold one resource URI, so Context.getByResourceURI can return several; ContextImpl.remove acts only on the ones that are lists. A Link, Reference or LinkReference rename keeps the type and resource-level ACL triples, updates inverse relations, fires EntryUpdated so the entry is re-indexed, and answers 500 instead of reporting success when the store fails; PUT /\{context-id\}/entry/\{entry-id\} rewrites the old resource URI throughout the submitted graph. Entry.removeAllowedPrincipalsFor returns whether the principal was listed; Entry.hasAllowedPrincipals throws RepositoryException instead of NullPointerException when the store is unreachable; EntryImpl.updateAllowedPrincipalsFor is removed

[ENTRYSTORE-835](https://metasolutions.atlassian.net/browse/ENTRYSTORE-835) Add a facetLang parameter to /search?type=solr, so a client can show facets for one language. Facet buckets are unchanged in shape \(a name and a count\), and for a request without facetLang the first Solr request is exactly the one sent before; later result-fill passes now run with faceting switched off. With facetLang set, the buckets of a literal facet field \(metadata.predicate.literal\_s.\* or the metadata.predicate.literal.\* shorthand, and their related. twins\) are narrowed to the labels occurring in that language or untagged, using RFC 4647 basic filtering so en also matches en-GB. The selection happens in Solr before facetLimit. A bucket's count is the label's count in all languages, taken before access filtering, not the number of entries in the requested language. Candidates are gathered with a bounded overrequest ranked by their count in the requested language while the final order uses the total count, so a label that is rare in that language but frequent overall can be missing from the top N entirely. Labels longer than 256 characters carry no language term and are therefore absent whenever facetLang is set. Internally the indexer writes one metadata.predicate.literal\_l.\* term per \(label, language\) pair, normalised with RDF4J Literals.normalizeLanguageTag so en-gb and en-GB share a term; that family is never accepted in facetFields or sort and never appears in a response. Startup now refuses to run against a Solr whose schema lacks those dynamic fields, or that answers 401/403 or 404 \(no such core\) when asked for the schema, instead of wiping the index and rebuilding it empty; an unreachable Solr keeps the previous behaviour, and if documents are then rejected during the initial reindex the version markers are withheld so the next start checks the schema again. Also fixed: a query needing more than one result-fill pass reported each facet field once per pass and asked Solr to recompute the facets on every pass. **Upgrade note:** deploy the updated Solr schema.xml and reload the core before starting 6.1. The indexer writes the new fields for every literal, and a schema without them rejects the whole document as an unknown field. With entrystore.data.folder configured the Solr version marker rebuilds the index on first boot; without it, and for a developer instance already running a 6.1-SNAPSHOT, start once with entrystore.solr.reindex-on-startup=on.
...
[ENTRYSTORE-1055](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1055) Consolidate the REST layer's policy-denial exception onto a single type. UnauthorizedException is removed and its three call sites now throw ForbiddenException; both were handled by the same handler with an identical mapping, so the choice between them carried no information and merely invited call sites to imply a status they did not control. No HTTP status changes: the status has always been a function of the caller \(401 with the bare reason phrase for anonymous callers, 403 with the call-site message for authenticated ones\), never of the exception type. ContextController and RelationController now set Last-Modified and ETag through the mandated HttpUtil helper rather than by hand — same headers, same values, and a null modification date is now logged rather than throwing \(ContextController\) or silently skipped \(RelationController\).

[ENTRYSTORE-1056](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1056) Decompose the REST module's ResourceController and ResourceService god classes and fix package placement. ResourceController is now a thin controller: the GET dispatch over syndication, file, string and graph resources, the feed-to-XML conversion and the modification-date updates live in ResourceService and SyndicationService. ResourceService itself is split by domain into ListResourceService \(children updates, moveEntry, ZIP import\), FileResourceService \(binary uploads and downloads\), UserService.updateSettings \(the user-settings block including password policy, session expiry and confirmation email\) and ProxyService.deleteUrl \(the SSRF-guarded remote DELETE\). SearchController hands its Solr limits and the filterQuery parsing to SearchService and SolrSearchInputValidator. Package placement: the MVC Converters move from model/api to model/api/converter, util/ResourceJsonSerializer becomes service/ResourceSerializationService, and the nine servlet filters previously split between configuration/ and security/ move to filter/. EntryService is Entry-first like the other entry-scoped services, RelationController.getResource is renamed getRelations \(springdoc operationId changes accordingly\), and the metadata and SSO login endpoints get accurate Swagger descriptions. **Behaviour note:** four observable fixes were made while moving code. A String resource without an rdf:value now answers 200 with an empty body instead of 500. A stored mimetype that does not parse is served as application/octet-stream instead of failing the download. The raw-body binary PUT now enforces entrystore.data.max-file-size like the multipart upload already did \(oversized bodies answer 400 instead of being stored\). moveEntry/fromList accept any absolute URI, not only http://, so HTTPS deployments no longer get absolute URIs prefixed with the base URL. Pre-existing issues found during the work are tracked as ENTRYSTORE-1107 \(500 on a malformed percent-escape in filterQuery\), ENTRYSTORE-1108 \(unvalidated entrystore.solr.max-limit\) and ENTRYSTORE-1109 \(PUT/DELETE metadata with rev\)

[ENTRYSTORE-1036](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1036) Evaluate the REST layer's Caffeine caches against Spring's cache abstraction and consolidate them. None of the caches is a read-through cache: the SAML and OAuth2 request repositories need an atomic get-and-remove, the rate limiters and login-attempt counters an atomic compute, the lockout cache a per-entry expiry, and the basic-auth password-verification cache a deduplicating loader that never stores a failed match — none of which the Spring Cache API expresses — so every cache stays a natively built Caffeine cache registered with the CacheManager through CaffeineCacheSource, and @EnableCaching is deliberately not enabled. Sign-up and password-reset confirmation tokens move from a hand-swept ConcurrentHashMap to a Caffeine cache that expires each token at its own deadline and is capped at 100 000 entries, and are listed as signup-tokens under GET /management/caches. **Behaviour note:** like every listed cache, an admin DELETE /management/caches/signup-tokens now clears it, which invalidates every pending sign-up and password-reset confirmation. The two SAML caches \(saml2-authn-requests, saml-auth-state\) are capped at 10 000 entries with a throttled WARN on capacity eviction, matching their OIDC counterparts. **Behaviour note:** beyond 10 000 concurrent in-flight SAML logins Caffeine evicts a recently admitted request \(W-TinyLFU, not the oldest\) and its callback fails as authentication-request-not-found, the posture the OIDC caches have had since they were introduced. The SAML ACS callback now removes its authentication request with an atomic remove, matching the OAuth2 twin, so two concurrent callbacks carrying the same RelayState cannot both take it out of the cache; Spring's ACS filter still loads the request non-destructively before removing it, so this is not by itself a replay guard

[ENTRYSTORE-1052](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1052) Migrate the remaining legacy Config reads in the REST layer to Spring configuration binding. Every key keeps its name; what changes is how a handful of them are parsed. **Config note — indexed list settings.** entrystore.auth.password.whitelist|blacklist, entrystore.auth.permitted.redirects, entrystore.proxy.whitelist.local|anonymous, entrystore.proxy.remote-resource.delete.whitelist and entrystore.auth.signup.whitelist now bind through Spring rather than Config.getStringList, and several shapes that used to mean something else are the reason to audit these keys before upgrading. A gap in the numbering no longer truncates the list: .1 plus .3 previously yielded only .1, because counting stopped at the first missing index, and a list that did not start at .1 yielded nothing at all — every entry now applies. Two spellings the previous reader could never match now bind for the same reason: an entry at .0, which it never probed because it started counting at .1, and a zero-padded index such as .01, which is not the literal .1 it looked for. And a bare, un-indexed value no longer wins: previously "whitelist=a" alongside "whitelist.1=b" resolved to a alone and the indexed entries were ignored, whereas the bare value is now dropped and the indexed entries apply instead, so the effective list becomes a different set rather than a longer or shorter one; a bare value with no indexed entries at all no longer binds. Since every one of these keys is an allowlist or a denylist, either shape makes the effective list differ from the one the previous release applied: for the allowlists that means admitting a caller it excluded — a username for local password login, a post-login redirect target, or a proxy host that is thereby exempted from the SSRF blacklist and the loopback/site-local/link-local address checks — while for entrystore.auth.password.blacklist the change is fail-closed, since extra entries can only ever block a superset. Startup therefore aborts on every one of these shapes — and on an entry with a non-numeric index suffix \(e.g. whitelist.local.l=...\), which was inert before and would bind as an active entry now — with an error naming the key, the entries whose meaning changed and the renumber-from-.1 remedy. The legacy-shape rules apply to the dotted and container environment-variable spellings, the only forms Config.getStringList could ever have been fed; a Spring-native bracketed list \(whitelist\[0\], a YAML sequence, a CLI --key\[0\]= override\) is exempt from them, because the previous reader could not parse that form at all and nothing about it changed meaning. A non-numeric suffix is a startup failure in every spelling, bracketed included. All of these keys are documented with the required indexed form in entrystore.properties\_example. The traversal predicate and blacklist lists \(entrystore.traversal.&lt;profile&gt;.N and entrystore.traversal.&lt;profile&gt;.blacklist.N\) moved to the same binding and carry the same gap divergence, but are not covered by the startup report because their profile names are operator-chosen; additionally a bare entrystore.traversal.&lt;profile&gt;=... no longer registers a profile at all, so a request naming it returns 400 instead of traversing. A bare entrystore.traversal.&lt;profile&gt;.blacklist=... is the one bare value still honoured — as a single denylist tuple, with a WARN recommending the indexed form — because dropping it would make the traversal denylist fail open. **Upgrade note:** before upgrading, check every key listed above for a bare value, an index gap, a .0 entry, zero-padded indices and non-numeric suffixes in the dotted or environment-variable form, and renumber contiguously from .1 — the application will not start until these are fixed. A list already written as a bracketed or YAML sequence needs no change. **Config note — boolean parsing.** entrystore.nonadmin.group-context-creation, entrystore.http.allow-content-disposition-inline, entrystore.auth.password.require-current-password and entrystore.auth.temp.lockout.admin are now parsed with strict boolean semantics \(true/on/yes/1 enable, false/off/no/0 disable\), matching entrystore.trust.x-forwarded-for in 6.0. The previous reader recognised only on and true as enabling and treated everything else as disabled, so this changes two things rather than one: yes and 1 now enable a setting they previously disabled — silently, with no diagnostic — and any value outside the recognised set now aborts startup instead of meaning false. Concretely, entrystore.nonadmin.group-context-creation=1 goes from admin-only to letting every authenticated user create group contexts. **Upgrade note:** audit these four keys for the values yes and 1, which change meaning, and for any value outside true/false/on/off/yes/no/1/0, which now blocks startup. The boolean keys reported by GET /management/status/extended \(entrystore.data.quota, entrystore.harvester.oai\[.multithreaded\], entrystore.repository.provenance, entrystore.solr\[.reindex-on-startup\], entrystore.backup.maintenance, entrystore.auth.signup, entrystore.auth.password-reset\) are unchanged: core still reads them, each with its own literal comparison, and the status report resolves every key with the predicate its actual consumer uses — data.quota, repository.provenance, solr and harvester.oai.multithreaded are enabled only by the literal on, harvester.oai is enabled by anything but the literal off, and solr.reindex-on-startup and backup.maintenance keep the legacy on/off/true/false parse — so the report cannot disagree with the behaviour it reports on and a stray spelling such as entrystore.solr=enabled cannot abort startup over a report-only value. Note that entrystore.auth.signup and entrystore.auth.password-reset currently have no runtime consumer at all — they only change the value reported by the extended status endpoint; nothing gates self-registration on them. **Behaviour note.** No default changes: the syndication feed cap for an unset entrystore.solr.max-limit remains 50. That key is now documented in entrystore.properties\_example, including the pre-existing quirk that its two consumers disagree when it is unset — search falls back to 100 and syndication to 50 — so setting it explicitly is what gives both the same cap

[ENTRYSTORE-1054](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1054) Send email through Spring's JavaMailSender instead of a static per-call jakarta.mail Session. Security note: transport security is entrystore.smtp.security — entrystore.properties\_example documented entrystore.smtp.ssl instead, a key the code never read, up to and including 5.6.2 and in the Spring Boot module's copies, which have never shipped in a tagged release, so deployments configured from those files were silently sending plaintext SMTP. 5.7.0 through 5.8.0 documented the canonical key correctly. Both spellings are now honoured: security is used whenever it resolves, ssl only as a fallback, and if both are set and disagree, security applies and the deprecated key is reported at WARN. Recognised values are matched the way Spring Boot's relaxed binding matches enums \(letters and digits only, case-insensitive\), so START\_TLS and start-tls resolve as starttls does. Neither key was validated before 6.1 — ssl was never read at all, and security \(read since at least 4.1, the earliest tag in the repository\) was compared with equalsIgnoreCase, so any unrecognised value silently meant plaintext — and a value that still cannot be resolved is now reported at ERROR and blocks sending entirely rather than being sent in the clear under a configuration meant to be encrypted. Startup is never aborted over it. A configured host that ends up on plaintext without the operator declaring it is warned about at startup rather than passing silently; entrystore.smtp.security=off declares it and silences that warning. **Upgrade note:** transport security is now enforced rather than downgraded, so a deployment carrying a stale entrystore.smtp.ssl value, or an unrecognised value in either key, will see sends start failing rather than continue insecurely. entrystore.smtp.ssl=ssl against the entrystore.smtp.port=587 the old example file documented alongside it now means implicit TLS on a submission port, and entrystore.smtp.ssl=starttls against a relay that does not advertise STARTTLS now throws instead of silently downgrading; a value neither key can resolve, such as security=tls or security=none, refuses to send at all. Signup, password reset, POST /message and the password-change confirmations all stop working in each of those cases. Verify the MTA supports the implied mode before upgrading, and declare a deliberately plaintext relay explicitly with entrystore.smtp.security=off. A second security fix: a CR or LF in the subject of POST /message is now collapsed to a space, closing an email-header injection an authenticated caller could otherwise use to add headers to a message carrying the deployment's own From address, and the from-address, the recipient and a caller-supplied Reply-To must now each be exactly one mailbox — no comma-separated list, no RFC822 group address \(g:a@x.tld,b@y.tld; parses as a single address that the SMTP transport then expands back into one envelope recipient per member\) and no CR or LF — because InternetAddress.parse accepts a list and a principal name is never format-validated, so a renamed principal could previously relay attacker-authored mail to arbitrary third parties under the deployment's own From. The operator-configured entrystore.smtp.email.bcc and entrystore.smtp.email.reply-to may still each be a list, as they have been since 2019. Config note: entrystore.smtp.email.from|bcc|reply-to are now documented and supersede entrystore.auth.email.from|bcc, which remain readable as a deprecated fallback; the SMTP socket timeouts, TLS server-identity checking and total send-attempt count become configurable as entrystore.smtp.connection-timeout, .read-timeout, .write-timeout, .check-server-identity and .max-send-attempts \(1-10, default 3 — a total attempt count, not a retry count\), all defaulting to the values previously compiled in; entrystore.smtp.username and entrystore.smtp.password must both be set, and a half-set pair is reported at ERROR and disables SMTP authentication rather than failing startup. Operational limits that were previously hardcoded also become configurable: entrystore.echo.max-file-size \(the POST /echo payload cap, now also reported as echoMaxEntitySize by GET /management/status/extended instead of the placeholder -1\), and entrystore.proxy.max-redirects, .connect-timeout and .read-timeout for the two outbound-fetch paths \(GET /proxy and DELETE /\{context-id\}/resource/\{entry-id\}?proxy=true\), plus entrystore.proxy.max-response-size for GET /proxy alone, since the resource-DELETE path never reads a response body — again all defaulting to the previously compiled-in values. Both size caps are additionally bounded from above \(512MB for the proxy response, 64MB for the echo payload\) because each body is held in memory for the duration of the request. Note that the proxy read-timeout is a per-socket-read deadline rather than a deadline for the response, so it does not bound how long a request thread can be held; what bounds the read loop is max-response-size. Validation and body binding are modernised on the same endpoints: POST /auth/signup now binds a typed request body with Bean Validation constraints instead of an untyped map, and a dedicated MethodArgumentNotValidException handler keeps the HTML error responses those endpoints have always returned. Security note: that handler also stops a validation failure from echoing internal identifiers, on both of the shapes Bean Validation produces. A request-body failure \(reachable unauthenticated on POST /message, since validation runs before the endpoint's own guest check\) carried the failing Java method signature, the request-body class name, the rejected value and Spring constraint codes; a request-parameter failure on a @Validated controller \(reachable unauthenticated on GET /search\) carried the controller method name. Both now carry only the failed constraint message, which changes the GET /search 400 body from "findEntriesSparql.query: 'query' param length must be minimum 3" to "'query' param length must be minimum 3". A second fix: RecaptchaVerifier no longer writes the reCaptcha private key into the log, which it did at DEBUG level on every verification by logging a URL it then never requested. Email-address validation is now available declaratively as @ValidEmail with the same rules as before, and the sign-up address is normalised with Locale.ROOT so the domain whitelist comparison cannot be defeated by a locale whose lower-casing differs from the root one. The user-settings and reCaptcha response bodies move from org.json to Jackson, keeping the existing absent-versus-explicit-null distinction for every user setting. One deliberate tightening there: a `disabled` value that is neither a boolean nor the strings `true`/`false` is now rejected with 400 instead of resolving to false, which previously answered 200 while leaving an account enabled — or silently re-enabled an already-disabled one

[ENTRYSTORE-1091](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1091) Fail RDF serialization loudly instead of serving truncated graphs. Behaviour note: a mid-stream RDF writer failure now returns 500 instead of 200 with a partial document, and endpoints that previously answered 400 or 406 for the same server-side failure \(entry, metadata, lookup and resource retrieval\) now also return 500. Scope note: this covers the RDF-document serialization paths only — the JSON-object assembly path selected by the entry endpoint's rdfFormat parameter still returns empty objects for formats other than application/json and application/ld+json, tracked separately as [ENTRYSTORE-1093](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1093)
...
[ENTRYSTORE-907](https://metasolutions.atlassian.net/browse/ENTRYSTORE-907) Update URISplit.java to accept URI with base URI and search params only
