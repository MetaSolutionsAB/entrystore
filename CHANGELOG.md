...
[ENTRYSTORE-1190](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1190) Remove entrystore.trust.x-forwarded-for, which had no effect: with server.forward-headers-strategy=framework, Spring's ForwardedHeaderFilter already makes request.getRemoteAddr\(\) the leftmost X-Forwarded-For entry and removes the header, so the sign-up, password-reset and search rate limiters were keyed by X-Forwarded-For with either setting. They now use request.getRemoteAddr\(\) directly. **Deployment note:** the reverse proxy must strip client-supplied X-Forwarded-For \(and Forwarded/X-Forwarded-\*\), otherwise clients can spoof the IP used for rate limiting, reCAPTCHA verification, lastUsedIpAddress and request logging. A configured entrystore.trust.x-forwarded-for is now ignored.
