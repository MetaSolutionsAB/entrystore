...
[ENTRYSTORE-1060](https://metasolutions.atlassian.net/browse/ENTRYSTORE-1060) Cache each user's group memberships across authorization decisions. Every access check used to resolve the caller's groups by iterating all entries in the \_principals context and testing membership on every group — twice per ACL evaluation and up to twelve times per entry serialization — so the cost of authorizing an authenticated request grew linearly with the size of the user directory. The core now keeps a per-repository map from user to group set, consulted on every decision and invalidated by a repository-event listener: any event sourced from a group entry \(membership add or remove, a full replacement of the member list, group create, rename or delete\) clears the whole cache, and a deleted user evicts its own entry, so a revoked membership is denied on the very next request. An epoch counter stops a scan that started before a membership change committed from being published after it. Measured on the same design under ENTRYSTORE-1085 at a 2 207-principal directory: group-authorized reads 16.6x faster \(1.69 ms to 0.10 ms per decision\), on par with admin reads, with no measurable write-path cost. **Config note:** entrystore.auth.group-cache \(default true\) is the kill switch; false restores the per-decision scan
