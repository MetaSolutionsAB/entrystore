...
[ENTRYSTORE-835](https://metasolutions.atlassian.net/browse/ENTRYSTORE-835) Add language information to facet values. A facet on a literal field \(metadata.predicate.literal_s.* or the metadata.predicate.literal.* shorthand\) keeps one bucket per label with unchanged counts, and each bucket now carries a "lang" array with the languages the label occurs in within the result set \(omitted when the label only occurs untagged\). The new facetLang parameter \(a BCP 47 tag, e.g. sv or en-GB\) keeps only the buckets whose label occurs in that language, with RFC 4647 prefix matching so en also matches en-GB, or untagged; the selection happens on the server before facetLimit, so the top N is taken within the requested language. The information comes from a new internal Solr field family metadata.predicate.literal_l.* \(and related.metadata.predicate.literal_l.*\) that stores label and normalised language tag for every literal of up to 256 characters; it is not accepted in facetFields or sort. **Upgrade note:** deploy the updated Solr schema.xml and reload the core before starting 6.1: the schema has no catch-all field, so an old schema rejects every document with a literal. EntryStore now checks the schema at startup and refuses to start when the new fields are missing, and a reindex during which Solr rejected documents no longer records the index as current. With entrystore.data.folder configured the index is rebuilt automatically on first boot by the Solr version marker; without it, and for a developer instance already running a 6.1-SNAPSHOT, start once with entrystore.solr.reindex-on-startup=on
