...
- Caches: build a Caffeine cache in the owning bean and expose it through `CaffeineCacheSource`, which registers it with the `CacheManager` so it is listed under `/management/caches`. The REST layer does not use `@EnableCaching` or `@Cacheable`: its caches need atomic compute or get-and-remove, per-entry expiry, or loader dedup with null-not-stored, none of which the Spring `Cache` API expresses (evaluated in ENTRYSTORE-1036). A size-capped cache written on an anonymous path should report capacity eviction through `CapacityEvictionWarning`; today the SAML/OIDC request repositories, the SAML/OIDC auth-state caches and `signup-tokens` do, while `FixedWindowRateLimiter`, the two `LoginAttemptService` caches and `CachingPasswordEncoder` do not yet. Core-layer caches such as the user-to-groups cache in `PrincipalManagerImpl` are invalidated inline or through a `RepositoryListener` and are not listed under `/management/caches`.
...
This project targets the latest available models of whichever coding agent is in use — no pinned version. Write prompt content (agents, skills, instruction files) to be safe under the most literal instruction interpretation, so it works unchanged across model upgrades:
...
- State subagent-launch rules explicitly (which agents, one launch call each, all in the same turn)
