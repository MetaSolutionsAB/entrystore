...
- `BaseSpec` also starts a shared **WireMock** server (dynamic port) for stubbing external HTTP services so ITs never depend on a live network. Today it stubs the reCAPTCHA `siteverify` endpoint with an always-success response; the corresponding `entrystore.auth.recaptcha.url` is injected into the Spring Boot app via command-line args. **Any `Zzz*` IT that exercises `/auth/signup` or `/auth/pwreset` must include `'--entrystore.auth.recaptcha.url=' + BaseSpec.getRecaptchaStubUrl()` in its `args` array.** A spec that needs to test an upstream failure can re-stub with `wireMockServer.stubFor(...)` and reset in `cleanup:` via `wireMockServer.resetMappings()` followed by `BaseSpec.registerDefaultRecaptchaStub()`.
