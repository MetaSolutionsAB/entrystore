...
- Caches: build a Caffeine cache in the owning bean and expose it through `CaffeineCacheSource`, which registers it with the `CacheManager` so it is listed under `/management/caches`. The REST layer does not use `@EnableCaching` or `@Cacheable`: its caches need atomic compute or get-and-remove, per-entry expiry, or loader dedup with null-not-stored, none of which the Spring `Cache` API expresses (evaluated in ENTRYSTORE-1036). A size-capped cache written on an anonymous path reports capacity eviction through `CapacityEvictionWarning`.
