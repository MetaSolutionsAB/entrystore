...
- Read config via Spring Boot mechanisms (`@Value("${prop:default}")` or `@ConfigurationProperties`); do not call `repositoryManager.getConfiguration().get*` from a service.
