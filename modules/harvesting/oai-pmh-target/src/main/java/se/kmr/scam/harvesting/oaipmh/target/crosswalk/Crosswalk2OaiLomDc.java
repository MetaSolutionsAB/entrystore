...
/**
 * Converts SCAM Entry and its metadata graphs to LOM/DC.
 * 
 * @author Hannes Ebner
 */
public class Crosswalk2OaiLomDc extends Crosswalk {
	...
	private String convertGraphToRDFXML(Graph g) {
		...
		try {
			...
			prettyWriter.handleNamespace("lom", NS.lom);
			prettyWriter.handleNamespace("lomterms", NS.lomterms);
			prettyWriter.handleNamespace("lomvoc", NS.lomvoc);
			prettyWriter.handleNamespace("lrevoc", NS.lrevoc);
			prettyWriter.handleNamespace("oe", NS.oe);
			prettyWriter.handleNamespace("sc", NS.sc);
			prettyWriter.handleNamespace("vcard", NS.vcard);
