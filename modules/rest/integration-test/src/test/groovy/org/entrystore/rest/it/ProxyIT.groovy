...
class ProxyIT extends BaseSpec {
	...
	def 'GET /proxy should send the original Host header to upstream, not the pinned IP'() {
		when:
		def conn = EntryStoreClient.getRequest('/proxy?url=' + mockUrl('/echo-host'))

		then:
		conn.getResponseCode() == HTTP_OK
		conn.inputStream.text == "localhost:${mockPort}"
	}

	def 'GET /proxy should forward percent-escapes in path and query unchanged'() {
		when:
		def conn = EntryStoreClient.getRequest('/proxy?url=' + mockUrl('/echo-target/a%20b?q=x%2Fy&r=%25'))
