 ...
 * @see Guard
 */
public class BasicGuard extends Guard {
	...
	public int authenticate(Request request) {
		...
		int authentificationResult = super.authenticate(request); 
		
		if(authentificationResult == -1) {
			log.error("Username or password invalid!");
		} else if(authentificationResult == 0) {
			log.error("Username or password not found!");
		} else if(authentificationResult == 1) {
			// Jippi we have logged in.
		}
	...
	public boolean checkSecret(Request request, String identifier, char[] secret) throws AuthorizationException {	
		...
		try {
			...
			if (query.getFirst("auth_user") != null && query.getFirst("auth_password") != null) {
				...
				String cookie = URLDecoder.decode(cookies.getFirstValue("scamSimpleLogin"));
