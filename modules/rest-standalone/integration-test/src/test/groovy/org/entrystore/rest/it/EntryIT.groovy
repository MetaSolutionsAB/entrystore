...
class EntryIT extends BaseSpec {
	...
	def "POST /{context-id}?entrytype=reference should create a new reference entry with encoded resource and metadata links"() {
		given:
		def params = [entrytype: 'reference', resource: encodedUrl, 'cached-external-metadata': encodedUrl]
		...
		entryExtMetadata[0]['value'] == encodedUrl
	...
	def "POST /{context-id}?entrytype=linkreference should create a new reference entry with encoded resource and metadata links"() {
		given:
		def params = [entrytype: 'linkreference', resource: encodedUrl, 'cached-external-metadata': encodedUrl]
	...
	def "POST /{context-id}?entrytype=link&template=templateEntry with metadata in the body, should create a new link entry with local metadata combined with MD from template entry"() {
		...
		def templateEntryId = createEntry(contextId, templateEntryParams, templateEntryBody)
		assert templateEntryId.length() > 0

		def params = [entrytype: 'link',
					  resource : resourceUrl,
					  template : EntryStoreClient.baseUrl + '/' + contextId + '/entry/' + templateEntryId]
		def body = [metadata: [(newResourceIri): [
			(NameSpaceConst.DC_TERM_TITLE)    : [[
													 type : 'literal',
