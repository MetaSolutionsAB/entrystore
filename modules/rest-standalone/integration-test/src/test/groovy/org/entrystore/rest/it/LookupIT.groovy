...
class LookupIT extends BaseSpec {
	...
	def "GET /{context-id}/lookup on non-existing context should return 404"() {
		...
		then:
		// Should be HTTP_NOT_FOUND - Bug in Restlet version, context scoped search does not work correctly
		// bug created https://metasolutions.atlassian.net/browse/ENTRYSTORE-986
		conn.getResponseCode() == HTTP_OK
	}
