...
class SearchIT extends BaseSpec {
	...
	def "GET /search?type=solr with complex Solr query as non-admin user should return empty search results"() {
		when:
		def conn = EntryStoreClient.getRequest('/search?type=solr&query=id:randomNonExistingId+OR+description.pl:opissearch', 'user')
	...
	// TODO: fix - returns OK with authorization error msg in the body, should return 401
	def "GET /search?type=sparql&query=dc:title as guest should respond with unauthorized error"() {
		...
		conn.getContentType().contains('application/json')
		def respJson = JSON_PARSER.parseText(conn.getInputStream().text)
		respJson['offset'] == null
		respJson['results'] == null
		respJson['resource'] == null
		respJson['error'] != null
		respJson['error'].toString().contains('_guest+not+authorized+for%3A+ReadMetadata')
	}
	...
	// TODO: Fix - returns 200 and authorization error msg in the body, should return 401 or 400
	def "GET /search?type=sparql&query=dc:title&syndication=random-string as guest should return BAD-REQUEST 400 due to invalid syndication format"() {
		given:
		...
		conn.getResponseCode() == HTTP_OK
		conn.getContentType().contains('application/json')
		def resp = JSON_PARSER.parseText(conn.getInputStream().text)
		resp['error'] != null
		resp['error'].toString().contains('_guest+not+authorized+for%3A+ReadMetadata')
	}
	...
	def "GET /search?type=sparql&query=dc:title&syndication=random-string as admin should return BAD-REQUEST 400 due to invalid syndication format"() {
		...
		resp['error'] == 'Invalid feed type [random-string]'
	}
