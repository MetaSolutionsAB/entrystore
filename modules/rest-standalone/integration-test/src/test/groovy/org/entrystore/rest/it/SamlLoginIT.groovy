...
class SamlLoginIT extends BaseSpec {
	...
	def setupSpec() {
		def existingInstance = EntryStoreApplicationStandaloneJetty.getInstance()
		if (existingInstance != null) {
			existingInstance.shutdownServer()
		}
	...
	def cleanupSpec() {
		// Stop the SAML instance
		def samlInstance = EntryStoreApplicationStandaloneJetty.getInstance()
		if (samlInstance != null) {
			samlInstance.shutdownServer()
		}
		appStarted = false
	}
	...
	def '1. GET /auth/saml should start SAML authentication flow - redirect to IDP with SAMLRequest'() {
		...
		// Bug in redirect-with-GET (entrystore.auth.saml.idp.keycloak.redirect-method=get) produces invalid SAMLRequest - not DEFLATE-compressed, only Base64 encoded
		/*then: 'SP should redirect to IDP: 307 response code and location header set'
		connection.getResponseCode() == 307
		def redirectLink = connection.getHeaderField('Location')
		redirectLink.contains(keycloakTestRealmUrl)
		redirectLink.contains('SAMLRequest=')
		 */

		then: 'SP should redirect to IDP'
		...
		response.contains('<input type=\'hidden\' id=\'SAMLRequest\' name=\'SAMLRequest\' value=\'')
		...
		response.contains('<input type=\'hidden\' id=\'RelayState\' name=\'RelayState\' value=\'')
	...
	def '4. POST SAMLResponse back to Service Provider and complete authentication'() {
		...
		assert spCallbackUrl.contains('/auth/saml') : 'SP callback URL does not point to /auth/saml'
