...
public class LoginTokenCache extends TokenCache<String, UserInfo> {
	...
	public void renameUser(String oldUserName, String newUserName) {
		...
		synchronized (tokenCache) {
			for (Entry<String, UserInfo> e : tokenCache.entrySet()) {
				...
				if (oldUserName.equals(oldUi.getUserName())) {
	...
	public UserInfo registerUserInteraction(String token, HttpServletRequest request) {
		checkArgument(token != null, "Token must not be null");
		...
		userInfo.setLastUsedUserAgent(request.getHeader("User-Agent"));
