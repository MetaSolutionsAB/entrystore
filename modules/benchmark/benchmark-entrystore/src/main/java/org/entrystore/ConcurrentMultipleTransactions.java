...
/**
 * Concurrent-writer variant of {@link MultipleTransactions}/{@link MultipleTransactionsBatched}
 * (ENTRYSTORE-1084): the same EntryStore-API insert path, but the person list is split into
 * contiguous chunks written by N threads into the same context. Per-insert modulo sampling and
 * inter-contexts are not supported here — the mode exists to measure write-path lock contention
 * (e.g. the A1 Solr write-path decoupling), not per-insert latency.
 */
